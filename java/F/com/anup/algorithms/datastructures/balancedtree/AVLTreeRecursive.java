...
import F.com.anup.algorithms.datastructures.utils.TreePrinter;
import F.com.anup.algorithms.datastructures.utils.TreePrinter.PrintableNode;
...
//######################################################################
  ...
  private Node balance(Node node) {
    ...
    if (node.bf == -2) {
      ...
      // Left-Left case.>>>>>>>>>>. middle letter is child/third letter granchild ::both left
      if (node.left.bf <= 0) {
        return leftLeftCase(node);

        // Left-Right case.>>>>>>>>>>>>middle letter is child/third letter granchild : left/right
      } else {
      ...
      // Right-Right case.>>>>>>>>>>. middle letter is child/third letter granchild ::both right
      if (node.right.bf >= 0) {
        return rightRightCase(node);

        // Right-Left case.>>>>>>>>>>. middle letter is child/third letter granchild ::right/left
      } else {
  ...
  private Node rightRotation(Node node) {
	  //TAKE TWO POINTERS AND POINT AT A AND B;; VIDOE 5:27 MIN	
	  ////1. B=A.left  2. A.left= B.right  3. B.right = A;// FRONT =LINK 
    Node newParent = node.left;
    node.left = newParent.right;
    newParent.right = node;
    update(node);
    update(newParent);// to update parent of new node
    return newParent;
  }
