...
class Main
{
 ...
 {*/
     ...
     */
		...
		/*
		 * Node root = new Node(15); root.left = new Node(10); root.right = new
		 * Node(20); root.left.left = new Node(8); root.left.right = new Node(12);
		 * root.right.left = new Node(16); root.right.right = new Node(25);
		 * 
		 * int k = 4;
		 * 
		 * // find the k'th smallest node Node result = findKthSmallest(root, k);
		 * 
		 * if (result != null) { System.out.printf("%d'th smallest node is %d", k,
		 * result.data); } else {
